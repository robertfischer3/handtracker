...
class PlusMinusButtons():
    ...
    def draw(self, img):
        if self.visible:
            cv2.rectangle(img, (self.x1, self.y1), (self.x2, self.y2), self.back_color, cv2.FILLED)
            cv2.putText(img, "+", ((self.x1 + 12), (self.y1 + 35)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, self.btm_text_color, 2, cv2.LINE_AA)
            cv2.rectangle(img, ((self.x1 + 100), self.y1), ((self.x2 + 100), self.y2), self.back_color, cv2.FILLED)
            cv2.putText(img, "-", ((self.x1 + 112), (self.y1 + 35)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, self.btm_text_color, 2, cv2.LINE_AA)
            cv2.putText(img, self.label, (self.label_offset_x, self.y2),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, self.text_color, 2, cv2.LINE_AA)
            cv2.putText(img, str(self.current_value), (self.x2 + 150, self.y2),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, self.text_color, 2, cv2.LINE_AA)
        return img
    ...
    def plus_btn_click(self, x, y):
        if self.visible:
            if self.max_value > self.current_value:
                point = Point(x, y)
                bounding_box = create_rectangle_array((self.x1, self.y1), (self.x2, self.y2))
    ...
    def minus_btn_click(self, x, y):
        if self.visible:
            if self.min_value < self.current_value:
                point = Point(x, y)
                bounding_box = create_rectangle_array((self.x1 + 100, self.y1), ((self.x2 + 100), self.y2))
                if point_intersects(point, bounding_box):
                    self.current_value -= 1